...
public class TopicAdminActionService {
	...
	private String operating_delete(Map<String, Object> parameterMap,Map<String,String> operationInfoMap,MessageResources mr,Locale locale) {
		...
		for (Threads threads_old : threadsList) {
			...
			writeLog(parameterMap,currentForumName, threads_old.getSubject(), threads_old.getTid() + "","DEL",timestamp);
	...
	private String operating_move(Map<String,Object> parameterMap,Map<String,String> operationInfoMap,MessageResources mr,Locale locale){
		...
		String sql_selectForum = "SELECT fid, name, modnewposts, allowpostspecial FROM "+tableprefix+"forums WHERE fid=? AND status>0 AND type<>'group'";
		...
		Map<String,String> tid_moveThreadMap = new HashMap<String, String>();
		Map<String,String> tid_delThreadMap = new HashMap<String, String>();
		List<Map<String,String>> resultMapList = dataBaseDao.executeQuery("SELECT tid,action FROM "+tableprefix+"threadsmod WHERE status=1 AND (action REGEXP '^[m|r][0-9]+$' OR action='TDEL')");
		for(Map<String,String> temp : resultMapList){
			if(temp.get("action").equals("TDEL")){
				tid_delThreadMap.put(temp.get("tid"), "");
			}else{
		...
		for (Threads threads_old : threadsList) {
			...
			writeLog(parameterMap, currentForumName,threads_old.getSubject(),threads_old.getTid()+"", "MOV",timestamp);
		}
		
		
		
		sendMessageToAuthor(parameterMap, currentForumName,(String)parameterMap.get("url"), operationE2, threadsList,operationInfoMap,mr,locale);
		...
		if(expirationInteger==0){
			...
			Posts lastPosts = postsDao.getLastPosts(Short.valueOf(fid));
			StringBuilder builder = new StringBuilder("UPDATE jrun_forums AS f SET f.threads=f.threads-");
			builder.append(threadCount);
			builder.append(", f.posts=f.posts-");
			builder.append((replyCount+threadCount));
			builder.append(", ");
			if(lastPosts!=null){
				Integer tid = lastPosts.getTid();
				Threads lasThreads = ((ThreadsDao)BeanFactory.getBean("threadsDao")).findByTid(tid);
				if(lasThreads!=null){
					String lastpost = lasThreads.getTid()+"\t"+Common.cutstr(Common.addslashes(lasThreads.getSubject().replaceAll("\t", " ")), 40, null)+"\t"+lastPosts.getDateline()+"\t"+lastPosts.getAuthor();
					builder.append("f.lastpost='");
					builder.append(lastpost);
					builder.append("' ");
				}
			}else{
				builder.append("f.lastpost='' ");
			}
			builder.append("WHERE f.fid=");
			builder.append(fid);

			dataBaseDao.execute(builder.toString());
			
			
			
			
			Posts lastPosts_movto = postsDao.getLastPosts(moveto);
			StringBuffer hqlBuffer_moveto = new StringBuffer("UPDATE Forums AS f SET f.threads=f.threads+"+threadsList.size()+", f.posts=f.posts+"+(replyCount+threadsList.size())+", ");
			if(lastPosts_movto!=null){
				...
				String lastpost = lasThreads_movto.getTid()+"\t"+Common.cutstr(Common.addslashes(lasThreads_movto.getSubject().replaceAll("\t", " ")), 40, null)+"\t"+lastPosts_movto.getDateline()+"\t"+lastPosts_movto.getAuthor();
				hqlBuffer_moveto.append("f.lastpost='"+lastpost+"' ");
			}else{
				hqlBuffer_moveto.append("f.lastpost='' ");
			}
			hqlBuffer_moveto.append("WHERE f.fid="+moveto);
