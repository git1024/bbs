...
public class StatvarsService {
	...
	public Stats_manageStatisticVO getFinalStats_manageStatisticVO(String beforeIR,String uidFromRequest,MessageResources mr,Locale locale){
		...
		for(int i = 0;i<=maxmodworksmonthsInteger;i++){
			...
			TimeInfo timeInfo = stats_manageStatisticVO.new TimeInfo();
		...
		if(uidFromRequest.equals("")){
			...
			if(bool_needToUpdate){
				...
				for(int i = 0;i<list_members.size();i++){
					...
					uids += ", "+resultMap_members.get("uid");
				}
			...
			while(iterator_finalMap.hasNext()){
				...
				ContentInfo contentInfo = stats_manageStatisticVO.new ContentInfo();
		...
		modactioncode.add("RBL");modactioncode.add("PTS");modactioncode.add("RFS");
		modactioncode.add("RMR");modactioncode.add("BNP");modactioncode.add("UBN");
		modactioncode.add("REC");modactioncode.add("URE");
	...
	private String getForumCount(DataBaseDao dataBaseDao){
		List<Map<String,String>> rml = dataBaseDao.executeQuery("SELECT COUNT(*) AS ct FROM "+tableprefix+"forums WHERE type IN ('forum', 'sub') AND status>0");
		if(rml!=null && rml.size()>0){
	...
	private Map<String,String> getHotforumInfo(DataBaseDao dataBaseDao){
		List<Map<String,String>> rml = dataBaseDao.executeQuery("SELECT posts, threads, fid, name FROM "+tableprefix+"forums WHERE status>0 ORDER BY posts DESC LIMIT 1");
		if(rml!=null && rml.size()>0){
			return rml.get(0);
		}else{
			Map<String,String> tm = new HashMap<String, String>();
