...
public final class ForumInit{
	...
	public static Map<String, String> queryForumSetInfo(){	
		...
		int forumcount=Integer.valueOf(dataBaseDao.executeQuery("SELECT COUNT(*) count FROM jrun_forums WHERE status>0 AND threadcaches>0").get(0).get("count"));
		...
		List<Map<String,String>> crons=dataBaseDao.executeQuery("SELECT nextrun FROM jrun_crons WHERE available>'0' AND nextrun>'0' ORDER BY nextrun LIMIT 1");
