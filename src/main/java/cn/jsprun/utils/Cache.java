...
public final class Cache {
	...
	private static void setCache(String cname, String cachename,Map<String,String> settings)throws Exception {
		...
		for (String obj : cnames) {
			...
			if("forumlinks".equals(obj)||"onlinelist".equals(obj)){
				...
				List<Map<String,String>> forumList=dataBaseDao.executeQuery("SELECT f.fid, f.type, f.name, f.fup, ff.viewperm FROM jrun_forums f LEFT JOIN jrun_forumfields ff ON ff.fid=f.fid LEFT JOIN jrun_access a ON a.fid=f.fid AND a.allowview='1' WHERE f.status>0 AND f.type<>'group' ORDER BY f.type, f.displayorder");
				...
				table="stylevars";
				sql="SELECT sv.* FROM jrun_stylevars sv LEFT JOIN jrun_styles s ON s.styleid = sv.styleid AND (s.available=1 OR s.styleid='0')";
				...
				List<Map<String,String>> forums = dataBaseDao.executeQuery("SELECT ff.fid, ff.viewperm, ff.formulaperm FROM jrun_forumfields ff,jrun_forums f WHERE f.fid=ff.fid AND f.status>'0' AND ff.password=''");
	...
	private static void getDataList(String sql, String table,String cols,String conditions, String cachename, String cname, String prefix,boolean append) throws Exception {
		...
		if ("admingroups".equals(cname)) {
			...
			List<Map<String, String>> styles = dataBaseDao.executeQuery("SELECT s.styleid,s.name,s.available,s.templateid, t.directory as tpldir FROM jrun_styles s LEFT JOIN jrun_templates t ON s.templateid=t.templateid WHERE s.available=1 OR s.styleid='0'");
