		...
		try{
			if(submitCheck(request, "prunesubmit")){
				...
				if (sb != null && !sb.equals("")) {
					...
					Set<String> tidcount = tids.keySet();
					for(String tid:tidcount){
						int value = tids.get(tid);
						dataBaseService.runQuery("update jrun_threads set replies=replies-"+value+" where tid='"+tid+"'",true);
					}
