		...
		try{
			if(!submitCheck(request, "typedetailsubmit")&&!submitCheck(request, "typepreviewsubmit")){
				...
				if(submitCheck(request, "typedetailsubmit")){
					...
					String[] typeselect = request.getParameterValues("typeselect");
					if (typeselect != null) {
		...
		if(export!=null){
				...
				if(submitCheck(request, "stylesubmit")||submitCheck(request, "importsubmit")){
					if(stylesubmit!=null){
						...
						if(stylevarMap!=null&&stylevarMap.size()>0){
							StringBuffer sql=new StringBuffer();
							sql.append("INSERT INTO jrun_stylevars (styleid, variable,substitute) VALUES ");
							Set<String> variables=stylevarMap.keySet();
							for (String variable : variables) {
								sql.append("('"+styleidnew+"', '"+variable+"','"+Common.addslashes(stylevarMap.get(variable))+"'),");
							}
							sql.deleteCharAt(sql.length()-1);
							dataBaseService.runQuery(sql.toString(),true);
						}
			...
			if(edit != null){
				...
				if (stylevars != null) {
					...
					Set<String> keys=stylevarsubsMap.keySet();
					for (String key : keys) {
						String value=stylevarsubsMap.get(key);
						...
						subsMap.put(key,background.toString());
					}
