...
public class RedirectManageAction extends BaseAction {
	...
	public ActionForward lastpost(ActionMapping mapping, ActionForm form,
		...
		String sql="";
		if(tid>0){
			sql="SELECT tid, replies, special FROM jrun_threads WHERE tid='"+tid+"' AND displayorder>='0'";
		}else{
			...
			sql="SELECT tid, replies, special FROM jrun_threads WHERE fid='"+fid+"' AND displayorder>='0' ORDER BY lastpost DESC LIMIT 1";
		}
