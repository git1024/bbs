...
public class PostManageAction extends BaseAction {
	...
	public ActionForward newthread(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		...
		this.common(request, response, settings, usergroups, forumfield, forum,special,access,fileupload);
					...
					if (attach != null) {
						...
						for (String attachname : attach) {
							if (attachname != null && !attachname.equals("")) {
								String errorMessage = checklowerlimit(request,extcredits,postattachcredits, checklowerlimits, member, 1);
	...
	public ActionForward newtrade(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		...
		Map<Integer, Integer> lowerlimits = (Map<Integer, Integer>) creditspolicy.get("lowerlimit");
		boolean checklowerlimits=(lowerlimits != null && lowerlimits.size() > 0);
		String lowerlimitmessage=this.checklowerlimit(request,extcredits, postcredits, checklowerlimits, member, 1);
		...
		try{
			if(submitCheck(request, "topicsubmit")){
				...
				if (tradetypes!=null&& optiondata != null&& optiondata.size() > 0) {
					...
					for (Integer optionid : keys) {
						if(flag){
							sql.append(",('"+ tradetypeid + "', '" + pid + "', '" + optionid + "', '"+ Common.addslashes(optiondata.get(optionid)) + "')");
						}else{
							sql.append("('"+ tradetypeid + "', '" + pid + "', '" + optionid + "', '"+ Common.addslashes(optiondata.get(optionid)) + "')");
						}
						flag=true;
	...
	public ActionForward showSmilies(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		...
		if(smileytypes!=null&&smileytypes.size()>0){	
			...
			for (String obj : typeids) {
				smileytypeMap.put(obj, dataParse.characterParse(smileytypes.get(obj), true));
			}
		...
		if(!Common.empty(id)&&dtype!=null&&dtype.size() > 0){
			...
			for(Integer optionid:dtypes){
				...
				if(option.get("type").equals("radio")){
					...
					radiomap.put(Common.toDigit(optiondata.get(optionid+"")), "checked=\"checked\"");
					options.put("value", radiomap);
				}else if(option.get("type").equals("select")){
					Map selectmap = new HashMap();
					selectmap.put(Common.toDigit(optiondata.get(optionid+"")), "selected=\"selected\"");
					...
					for(String value:values){
						checkboxmap.put(Common.toDigit(value), "checked=\"checked\"");
					}
	...
	public ActionForward reply(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		...
		if((thread.getClosed()>0||checkautoclose(thread,forum)) && !modertar){
		...
		if (request.getParameter("replysubmit") == null || previewpost!=null) {
			...
			this.common(request, response, settings, usergroups,fileds, forum, thread.getSpecial(),access,fileupload);
	...
	public ActionForward toedit(ActionMapping mapping, ActionForm form,HttpServletRequest request, HttpServletResponse response) {
		...
		if (thread == null) {
			...
			Map<String,String> access = accesslist.size()>0?accesslist.get(0):null;
			this.common(request, response, settings, usergroups, forumfield, forums,special,access,null);
			if (post.getFirst() == 1) {
						...
						for (String typeid : typeids) {
							if(forumtradetypes.size()==0 || forumtradetypes.containsValue(typeid)) {
								if(typeid.equals(trade.get("typeid"))){
									tradetypeselect.append("<option value=\""+typeid+"\" selected=\"selected\">"+Common.strip_tags(tradetypesmap.get(typeid))+"</option>");
								}else{
									tradetypeselect.append("<option value=\""+typeid+"\">"+Common.strip_tags(tradetypesmap.get(typeid))+"</option>");
								}
							}
	...
	@SuppressWarnings("unchecked")
	public void common(HttpServletRequest request,HttpServletResponse response, Map<String, String> settings,Map<String, String> usergroups,Forumfields forumfield, Forums forum, int special,Map<String,String> access,FileUploadUtil fileupload) {
		int timestamp = (Integer)(request.getAttribute("timestamp"));
		HttpSession session = request.getSession();
		Members member = (Members) session.getAttribute("user");
		MessageResources resources = getResources(request);
		...
		if(message!=null)		{
			...
			try {
				request.getRequestDispatcher("/templates/default/nopermission.jsp").forward(request, response);
			} catch (Exception e) {
			} 
		}
		...
		boolean iscircle = !settings.get("supe_status").equals("0")&& !settings.get("supe_circlestatus").equals("0")&& forum.getStatus() == 2;
		...
		int secqaastatus = (Integer)secqaa.get("status");
		secqaa=null;
		request.setAttribute("secqaacheck", (secqaastatus & 2) > 0&& (minposts <= 0 || member.getPosts() < minposts));
		...
		if(smileytypes!=null&&smileytypes.size()>0)
		{
			Set<String> typeids=smileytypes.keySet();
			for (String typeid : typeids) {
				smileytypeMap.put(typeid, dataParse.characterParse(smileytypes.get(typeid), true));
	...
	private String checkAttachment(HttpServletRequest request,List filelist,Forumfields forumfield) {
		if(filelist.size()>0){
			...
			List<Map<String, String>> sumsizelist = dataBaseService.executeQuery("select sum(filesize) as size from jrun_attachments as a where a.uid="+ jsprun_uid + " and a.dateline+86400>=" + timestamp);
			String todaysize = "0";
				...
				if (fileitem!=null && fileitem.getSize()>0) {
					...
					if (flag) {
						if (convertInt(maxattachsize)==0 || fileitem.getSize() < convertInt(maxattachsize)) {
							...
							String delpath = servlet.getServletContext().getRealPath("/")+ attachment;
	...
	@SuppressWarnings("unchecked")
	private String checklowerlimit(HttpServletRequest request,Map extcredits,Map<Integer, Integer> creditsarray,boolean checklowerlimits, Members member, int coef) {
		String message = null;
