		...
		if (searchid != null && !searchid.equals("")) {
			...
			try{
				if(submitCheck(request, "searchsubmit")){
					...
					String pmsindexhql = "select searchid from jrun_pmsearchindex as p where p.searchstring = '"+ Common.addslashes(searchstring) + "' and p.expiration >" + timestamp;
	...
	public ActionForward sendorajax(ActionMapping mapping, ActionForm form,
		...
		try{
			if(submitCheck(request, "pmsubmit")){
				...
				message = Common.cutstr(message, 40000, "");
				subject = Common.htmlspecialchars(subject);
				Members members = memberService.findByName(msgto.trim());
