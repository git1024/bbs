...
<table cellspacing="0" cellpadding="0" width="100%">
	...
	<tbody>
		<c:forEach items="${polllists}" var="polllist"><tr>
			...
			<td><c:choose><c:when test="${polllist.displayorder==-1}"><bean:message key="forum_recyclebin"/></c:when><c:when test="${polllist.displayorder==-2}"><bean:message key="my_threads_aduit"/></c:when><c:when test="${polllist.close>=1}"><bean:message key="closed"/></c:when><c:otherwise><bean:message key="my_threads_common"/></c:otherwise></c:choose></td>
		</tr></c:forEach>
