...
<div id="nav">
	...
	<c:choose>
		...
		<c:when test="${action=='supe_push'}"><bean:message key="admin_supe_push_addremove" /></c:when>
	...
	<div class="mainbox formbox">
		<h1>
			<c:choose>
				...
				<c:when test="${action=='supe_push'}"><bean:message key="admin_supe_push" /></c:when>
				<c:when test="${action=='removereward'}"><bean:message key="RMR" /></c:when>
				<c:when test="${action=='bump'}"><bean:message key="admin_bump_down" /></c:when>
				<c:when test="${action=='recommend'}"><bean:message key="REC" /></c:when>
			</c:choose>
		...
		<table summary="Operating" cellspacing="0" cellpadding="0">
			...
			<c:choose>
				...
				<c:when test="${action=='stick'}">
					<tr>
						...
						<td>
							...
							<label><input class="radio" type="radio" name="level" value="1" ${valueObject.level== '1'?'checked':''} onclick="$('expirationarea').disabled=0" /> <img src="images/default/pin_1.gif" alt="<bean:message key="admin_stick_1" />" /> <bean:message key="admin_stick_1" /></label>
							<c:if test="${valueObject.stickPurview>1}"><label><input class="radio" type="radio" name="level" value="2" ${valueObject.level== '2'?'checked':''} onclick="$('expirationarea').disabled=0" /> <img src="images/default/pin_2.gif" alt="<bean:message key="admin_stick_2" />" /> <bean:message key="admin_stick_2" /></label></c:if>
							<c:if test="${valueObject.stickPurview>2}"><label><input class="radio" type="radio" name="level" value="3" ${valueObject.level== '3'?'checked':''} onclick="$('expirationarea').disabled=0" /> <img src="images/default/pin_3.gif" alt="<bean:message key="admin_stick_3" />" /> <bean:message key="admin_stick_3" /></label></c:if>
						</td>
					</tr>
				</c:when>
				...
				<c:when test="${action=='supe_push'}">
					<tr>
						<td class="altbg1"><span class="bold"><bean:message key="threads_push_supesite" /></span></td>
						<td class="altbg2">
							<input class="radio" type="radio" name="supe_pushstatus" value="2" checked="checked"> <bean:message key="threads_push" /> &nbsp; &nbsp;
							<input class="radio" type="radio" name="supe_pushstatus" value="-2"> <bean:message key="RFS" /> &nbsp; &nbsp;
						</td>
					</tr>
				</c:when>
